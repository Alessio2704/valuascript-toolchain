...
namespace valuascript::compiler::test {
    TEST_F(AstBaseTest, ValidatesNestedFunctionCallsOnRHS) {
    ...
    TEST_F(AstBaseTest, ValidatesConditionalExpressionAsFunctionParameter) {
    ...
    TEST_F(AstBaseTest, ValidatesPrimitiveLiteralsAsParameters) {
    ...
    TEST_F(AstBaseTest, ValidatesMathAndLogicExpressionsAsParameters) {
    ...
    TEST_F(AstBaseTest, ValidatesPostfixExpressionsAsParameters) {
    ...
    TEST_F(AstBaseTest, ValidatesParenthesizedExpressionsAsParameters) {
    ...
    TEST_F(AstBaseTest, ValidatesDeepFunctionCallWithNamedArguments) {
        // This test proves maximum orthogonality: arguments can be primitives, tensors, math, or other function calls.

        auto ast = parse_code(
            "let val = run_model(target: \"AAPL\", projections: [100, 120, 140], wacc: base_rate + 0.02, stress_test: get_scenario(type: \"bear\"))");
