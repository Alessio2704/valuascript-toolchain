...
namespace valuascript::compiler::test
{
        ...
        for (auto& var_item : items)
        {
            ...
            cb.on_normal_branch = [](const ProcessingItem& item, const Context& ctx, int next_rec_depth)
                -> std::vector<ProcessingItem>
                {
                    ...
                    UniversalVerifier inner_verifier = (verifier_match && verifier_match->verifier.has_value())
                                                           ? verifier_match->verifier.value()
                                                           : item.verifier;
                    std::optional<std::vector<ParserExpectedError>> branch_errors =
                        (verifier_match && verifier_match->errors.has_value())
                    ...
                    if (match && !match->excluded_sentinels.empty())
                    {
                        ...
                        {
                            if (std::find(combined_excluded.begin(), combined_excluded.end(), k) == combined_excluded.
                                end())
                            {
                    ...
                    auto make_item = [&](const std::vector<SentinelKind>& accepted_for_item) -> ProcessingItem
                    {
                        ...
                        return ProcessingItem{
                            ...
                            .custom_errors = branch_errors,
                            .excluded_sentinels = combined_excluded,
                ...
                -> std::vector<ProcessingItem>
                {
                                         ...
                                         size_t seed_offset) -> ProcessingItem
                    {
                        ...
                        return ProcessingItem{
                            ...
                            .verifier = (match && match->verifier.has_value() && !is_injectable_inner_verifier)
                                            ? inner_verifier
                                            : (ctx.transform_verifier_block
                                                   ? ctx.transform_verifier_block(inner_verifier, pre, post)
                                                   : (ctx.transform_verifier
                                                          ? ctx.transform_verifier(inner_verifier)
                                                          : inner_verifier)),
                            .path_name = item.path_name + " -> " + (next_rec_depth > item.recursion_depth
                                                                        ? std::string(ctx.name) + "(Recurse)"
                                                                        : std::string(ctx.name)),
 ...
 "Column end mismatch on error: " << i << std::endl;
                                                       ...
                                                       ProgramSpec broken_part_spec)
    {
        ...
        ForEachRecoveryProgram(code, std::move(broken_part_spec), "", base_seed,
