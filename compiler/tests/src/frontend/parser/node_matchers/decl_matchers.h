...
namespace valuascript::compiler::test
{
    struct FunctionDefMatcher
    {
        ...
        {
            ExpectFunctionDef(f, name, modifiers, params, returns, body, docstring);
        }
    };

    inline FluentNodeMatcher<FunctionDefMatcher> IsFunctionDef(std::string name,
                                                              std::vector<ModifierSpec> modifiers = {},
                                                              std::vector<ParamSpec> params = {},
                                                              std::vector<TypeVerifier> returns = {},
                                                              std::vector<StmtVerifier> body = {},
    ...
    struct StructDefMatcher
    {
        ...
        {
            ExpectStructDef(s, name, modifiers, fields);
        }
    };

    template <typename T>
    StructVerifier IsStructDef(std::string, std::initializer_list<T>) = delete;
    template <typename T>
    StructVerifier IsStructDef(std::string, std::vector<ModifierSpec>, std::initializer_list<T>) = delete;

    inline FluentNodeMatcher<StructDefMatcher> IsStructDef(std::string name, std::vector<ModifierSpec> modifiers = {},
                                                          std::vector<FieldSpec> fields = {})
    {
    ...
    inline auto IsStructDef(std::string name, FieldSpecs&&... fields)
    {
    ...
    template <typename... FieldSpecs>
        requires (sizeof...(FieldSpecs) > 0 && !(sizeof...(FieldSpecs) == 1 && std::same_as<
            std::decay_t<std::tuple_element_t<0, std::tuple<FieldSpecs...>>>, std::vector<FieldSpec>>))
    inline auto IsStructDef(std::string name, std::vector<ModifierSpec> modifiers, FieldSpecs&&... fields)
    {
        std::vector<FieldSpec> field_list = {std::forward<FieldSpecs>(fields)...};
        return FluentNodeMatcher<StructDefMatcher>{StructDefMatcher{
    ...
    struct EnumDefMatcher
    {
        ...
        {
            ExpectEnumDef(e, name, modifiers, type, cases);
        }
    };

    template <typename T>
    EnumVerifier IsEnumDef(std::string, std::initializer_list<T>) = delete;
    template <typename T, typename U = TypeVerifier>
    EnumVerifier IsEnumDef(std::string, std::vector<ModifierSpec>, U&&, std::initializer_list<T>) = delete;

    template <TypeNodeMatcher T = TypeVerifier>
    inline FluentNodeMatcher<EnumDefMatcher> IsEnumDef(std::string name, std::vector<ModifierSpec> modifiers = {},
                                                      T&& type = nullptr,
                                                      std::vector<EnumCaseSpec> cases = {})
    {
    ...
    inline auto IsEnumDef(std::string name, EnumCaseSpecs&&... cases)
    {
    ...
    template <TypeNodeMatcher T = TypeVerifier, typename... EnumCaseSpecs>
        requires (sizeof...(EnumCaseSpecs) > 0 && !(sizeof...(EnumCaseSpecs) == 1 && std::same_as<
            std::decay_t<std::tuple_element_t<0, std::tuple<EnumCaseSpecs...>>>, std::vector<EnumCaseSpec>>))
    inline auto IsEnumDef(std::string name, std::vector<ModifierSpec> modifiers, T&& type,
                          EnumCaseSpecs&&... cases)
    {
        std::vector<EnumCaseSpec> case_list = {std::forward<EnumCaseSpecs>(cases)...};
        return FluentNodeMatcher<EnumDefMatcher>{EnumDefMatcher{
    ...
    struct TypeAliasMatcher
    {
        ...
        {
            ExpectTypeAlias(a, name, modifiers, target);
        }
    };

    inline FluentNodeMatcher<TypeAliasMatcher> IsTypeAlias(std::string name, std::vector<ModifierSpec> modifiers = {},
    ...
    struct ImportMatcher
    {
        ...
        {
            ExpectImport(i, modifiers, path);
        }
    };

    inline FluentNodeMatcher<ImportMatcher> IsImport(std::string path, std::vector<ModifierSpec> modifiers = {})
    {
    ...
    template <typename V = AnyMatcher>
    inline auto IsDirective(std::string name, V&& value = {})
    {
